...
Chicken::Chicken(Vector3 position1, int number1) : LameMob(position1, number1)
{
    SPD = 1.0f; //velocidad al caminar
    HP = 3.0f;

    legs = Vector3(-PIXEL*6,-6*PIXEL,0);

    SetCollideBox(Vector3(PIXEL*4,PIXEL*10,PIXEL*7),Vector3(PIXEL*3,PIXEL*6,PIXEL*7));
    ...
    mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::chickenTexture);
}
...
void Chicken::BoxCollision()
{
    for(int x = 0; x < world->mChickens.size(); x++)
    {
        Chicken *TestChicken2 = world->mChickens[x];
        if(TestChicken2->number != number && (TestChicken2->position.x != position.x || TestChicken2->position.z != position.z || TestChicken2->position.y != position.y))
        {
            if(TestChicken2->bBox.intersect(bBox))
            {
                float angle1 = ATAN2_fast(position.x-TestChicken2->position.x,position.z-TestChicken2->position.z); // super secret function
...
void Chicken::TakeDamage(float damage, float power, float dt)
{
    HP -= damage;
...
void Chicken::TakeDamageFromPointExplosion(float damage, float power, float dt, Vector3 point)
{
    explosionPos = point;
...
void Chicken::Update(CraftWorld *myWorld, Vector3 playerPos, float dt)
{
    world = myWorld;
        ...
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14+5)
        {
            ...
            if(dt < 0.1f)
            {
                ...
                if(killed == false)
                {
                    ...
                    if(handItemId != WheatSeeds::getID() || fastDistanceToPlayer >= 13)
                    {
                        headAngleY = 0.0f;
                ...
                if(kicked == true || handItemId == WheatSeeds::getID())
                {
...
void Chicken::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14 && camFrustum.BoxInFrustum(bBox) == Frustum::Intersects)
        {
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc = {PIXEL*5,PIXEL,0}; //+5
            sceGumTranslate(&loc);
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->chickenBodyVertices);
            sceGumPopMatrix();
			
			
			
            sceGumPushMatrix();
            ScePspFVector3 loc2 = {PIXEL*6,PIXEL*4,0};
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->chickenHeadVertices);
            ...
            if(DistanceToPlayer() < 40.0f)
            {
                ...
                sceGumRotateY(0.0f);
                sceGumRotateZ(0.0f);
                sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->chickenPicosVertices);
                sceGumPopMatrix();
            }
			...
			sceGumPushMatrix();
            ScePspFVector3 loc7 = {-PIXEL*1,-PIXEL*6,PIXEL*6};
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->chickenAlaVertices);
            sceGumPopMatrix();

            sceGumPushMatrix();//done
            ScePspFVector3 loc8 = {-PIXEL*1,-PIXEL*6,-PIXEL*1};
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc3 = {-PIXEL*1,-PIXEL*6,PIXEL*1}; //done i thing that
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->chickenLegVertices);
            sceGumPopMatrix();

            sceGumPushMatrix();
            ScePspFVector3 loc5 = {-PIXEL*1,-PIXEL*6,-PIXEL*2};
