...
Spider::Spider(Vector3 position1, int number1) : LameMob(position1, number1)
{
    legs = Vector3(-PIXEL*6,PIXEL*-2,0);
	
	//Valor orginal legs = Vector3(0,PIXEL*-6,0);

	// primera parte del SetCollideBox(Vector3(PIXEL*0,PIXEL*0,PIXEL*0)
	// x=direccion contraria y=abajo z=derecha 
	// x=direccion frontal y=arriba z=izquierda 

	//SetCollideBox(Vector3(PIXEL*12,PIXEL*5,PIXEL*8),Vector3(PIXEL*4,PIXEL*12,PIXEL*8));

    SetCollideBox(Vector3(PIXEL*3,PIXEL*5,PIXEL*8),Vector3(PIXEL*3,PIXEL*7,PIXEL*8));
    ...
    mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::spiderTexture);
}
...
void Spider::BoxCollision()
{
    for(int x = 0; x < world->mSpiders.size(); x++)
    {
        Spider *TestSpider2 = world->mSpiders[x];
        if(TestSpider2->number != number && (TestSpider2->position.x != position.x || TestSpider2->position.z != position.z || TestSpider2->position.y != position.y))
        {
            if(TestSpider2->bBox.intersect(bBox))
            {
                float angle1 = ATAN2_fast(position.x-TestSpider2->position.x,position.z-TestSpider2->position.z); // super secret function
...
void Spider::TakeDamage(float damage, float power, float dt)
{
    float AP = 2.0f;
    AP = 2.0f;
...
void Spider::TakeDamageFromPointExplosion(float damage, float power, float dt, Vector3 point)
{
    explosionPos = point;
...
void Spider::Haunting(CraftWorld *myWorld, float dt)
{
	world = myWorld;
...
void Spider::Update(CraftWorld *myWorld, Vector3 playerPos, float dt)
{
    world = myWorld;
    ...
    if(toDestroy == false)
    {
        ...
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14+5)
        {
            ...
            if(dt < 0.1f)
            {
                ...
                if(killed == false)
                {
					if(world->worldDayTime >= 21 || world->worldDayTime <= 6)
					{
...
void Spider::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
