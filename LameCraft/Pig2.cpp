...
Pig::Pig(Vector3 position1, int number1) : LameMob(position1, number1)
{
    SPD = 1.0f;
    HP = 10.0f;
    ...
    SetCollideBox(Vector3(PIXEL*7,PIXEL*11,PIXEL*7),Vector3(PIXEL*7,PIXEL*7,PIXEL*7));
        ...
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14+5)
        {
            ...
            if(dt < 0.1f)
            {
                ...
                if(killed == false)
                {
                    ...
                    if(handItemId != Carrot::getID() || fastDistanceToPlayer >= 13)
                    {
                        headAngleY = 0.0f;
...
void Pig::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14 && camFrustum.BoxInFrustum(bBox) == Frustum::Intersects)
        {
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->pigBodyVertices);
            sceGumPopMatrix();
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->pigHeadVertices);

            // nose drawing
            if(DistanceToPlayer() < 40.0f)
            {
                sceGumPushMatrix();
                ScePspFVector3 loc52 = {PIXEL*0.5f,PIXEL*4,0};
                sceGumTranslate(&loc52);

                sceGumRotateX(0.0f);
                sceGumRotateY(0.0f);
                sceGumRotateZ(0.0f);
                sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->pigNosesVertices);
                sceGumPopMatrix();
            }
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc3 = {-PIXEL*6,-PIXEL*6,PIXEL*3};
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc4 = {PIXEL*6,-PIXEL*6,PIXEL*3};
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc5 = {-PIXEL*6,-PIXEL*6,-PIXEL*3};
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc6 = {PIXEL*6,-PIXEL*6,-PIXEL*3};
